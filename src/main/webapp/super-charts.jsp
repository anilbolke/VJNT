...
<%
    ...
    // Division scope: "ALL"/empty aggregates across every division; otherwise one division.
...
<head>
    <meta charset="UTF-8">
    <title>Level Jump Analytics - All Divisions</title>
    <script src="https://cdn.jsdelivr.net/npm/chart.js@3.9.1/dist/chart.min.js"></script>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh; padding: 20px; }
        .container { max-width: 1400px; margin: 0 auto; }
        .header { background: white; padding: 25px; border-radius: 10px; margin-bottom: 30px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px; }
        .header h1 { color: #333; font-size: 28px; }
        .header-subtitle { color: #666; font-size: 14px; margin-top: 5px; }
        .header-controls { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; }
        #divisionFilter { padding: 10px 14px; border: 2px solid #667eea; border-radius: 6px; font-size: 14px; font-weight: 600; color: #4a4a8a; background: white; cursor: pointer; }
        .back-button { display: inline-block; padding: 10px 20px; background: #667eea; color: white; text-decoration: none; border-radius: 5px; font-weight: 600; cursor: pointer; border: none; transition: background 0.3s; }
        ...
        .chart-title { font-size: 18px; font-weight: 600; color: #333; margin-bottom: 20px; padding-bottom: 10px; border-bottom: 2px solid #667eea; }
        .chart-wrapper { position: relative; height: 400px; overflow: visible; }
        .chart-wrapper.schools-chart { height: 600px; overflow: visible; }
        .chart-container.schools-full-width { grid-column: 1 / -1; }
        .heatmap-container { grid-column: 1 / -1; background: white; padding: 25px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); }
        .heatmap-title { font-size: 18px; font-weight: 600; color: #333; margin-bottom: 20px; padding-bottom: 10px; border-bottom: 2px solid #667eea; }
        .heatmap { display: grid; gap: 2px; background: #f5f5f5; padding: 10px; border-radius: 5px; overflow-x: auto; }
        .heatmap-row { display: grid; grid-template-columns: 180px repeat(3, 1fr); gap: 2px; min-width: max-content; }
        .heatmap-cell { padding: 15px 10px; text-align: center; font-weight: 500; border-radius: 3px; color: white; font-size: 13px; min-width: 80px; display: flex; align-items: center; justify-content: center; word-wrap: break-word; white-space: normal; }
        .heatmap-cell.header { background: #667eea; color: white; font-weight: 600; font-size: 13px; }
        .heatmap-cell.label { background: #999; color: white; text-align: center; font-weight: 500; padding: 15px 5px; word-break: break-word; white-space: normal; font-size: 12px; }
        .heatmap-cell.low { background: #90EE90; color: #333; }
        .heatmap-cell.medium { background: #FFD700; color: #333; }
        .heatmap-cell.high { background: #FF6B6B; color: white; }
        .district-percentage-container { grid-column: 1 / -1; background: white; padding: 25px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); margin-bottom: 30px; }
        .district-percentage-title { font-size: 18px; font-weight: 600; color: #333; margin-bottom: 20px; padding-bottom: 10px; border-bottom: 2px solid #667eea; }
        .total-students-card { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; border-radius: 10px; margin-bottom: 20px; text-align: center; }
        .total-students-card .number { font-size: 48px; font-weight: bold; }
        .total-students-card .label { font-size: 16px; margin-top: 10px; opacity: 0.9; }
        .district-cards-wrapper { display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 20px; margin-top: 30px; }
        .district-card { background: white; padding: 25px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.08); border-top: 4px solid #667eea; transition: all 0.3s ease; position: relative; overflow: hidden; }
        .district-card:hover { box-shadow: 0 8px 16px rgba(0, 0, 0, 0.12); transform: translateY(-2px); }
        .district-card.high { border-top-color: #2ecc71; background: linear-gradient(135deg, rgba(46, 204, 113, 0.05) 0%, rgba(46, 204, 113, 0.02) 100%); }
        .district-card.medium { border-top-color: #f39c12; background: linear-gradient(135deg, rgba(243, 156, 18, 0.05) 0%, rgba(243, 156, 18, 0.02) 100%); }
        .district-card.low { border-top-color: #e74c3c; background: linear-gradient(135deg, rgba(231, 76, 60, 0.05) 0%, rgba(231, 76, 60, 0.02) 100%); }
        .district-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 15px; }
        .district-rank-badge { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 24px; font-weight: bold; box-shadow: 0 4px 8px rgba(102, 126, 234, 0.3); }
        .district-name-large { font-size: 20px; font-weight: 700; color: #333; margin-bottom: 15px; flex: 1; padding-right: 10px; }
        .district-stats-large { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px; }
        .stat-box { background: white; padding: 12px; border-radius: 8px; text-align: center; border: 1px solid #e0e0e0; }
        .stat-box .number { font-size: 24px; font-weight: bold; color: #667eea; }
        .stat-box .label { font-size: 12px; color: #999; margin-top: 5px; text-transform: uppercase; letter-spacing: 0.5px; }
        .percentage-bar-large { width: 100%; height: 28px; background: #e8e8e8; border-radius: 14px; overflow: hidden; display: flex; align-items: center; justify-content: flex-end; padding-right: 10px; margin-top: 10px; }
        .percentage-fill-large { background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); height: 100%; border-radius: 14px; display: flex; align-items: center; justify-content: center; color: white; font-size: 13px; font-weight: 700; padding: 0 12px; transition: width 0.5s ease; }
        .district-card.high .percentage-fill-large { background: linear-gradient(90deg, #2ecc71 0%, #27ae60 100%); }
        .district-card.medium .percentage-fill-large { background: linear-gradient(90deg, #f39c12 0%, #e67e22 100%); }
        .district-card.low .percentage-fill-large { background: linear-gradient(90deg, #e74c3c 0%, #c0392b 100%); }
        .performance-label { font-size: 12px; font-weight: 600; margin-top: 10px; padding: 6px 12px; border-radius: 20px; display: inline-block; margin-right: 10px; }
        .performance-label.high { background: #d5f4e6; color: #27ae60; }
        .performance-label.medium { background: #fce8cd; color: #e67e22; }
        .performance-label.low { background: #fadbd8; color: #c0392b; }
        .footer { background: white; padding: 20px; border-radius: 10px; text-align: center; color: #666; font-size: 13px; }
        @media (max-width: 768px) {
            .charts-grid { grid-template-columns: 1fr; }
            .header { flex-direction: column; text-align: center; gap: 15px; }
            .heatmap-row { grid-template-columns: auto repeat(3, 1fr) !important; }
        }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="header">
            <div>
                ...
                <p class="header-subtitle"><span id="scopeLabel"><%= scopeLabel %></span> | Total Students with Level Jumps: <%= levelJumpStudents.size() %></p>
            </div>
            <div class="header-controls">
                <select id="divisionFilter" onchange="onDivisionChange()" title="Select division scope">
                ...
                <a href="<%= request.getContextPath() %>/super-officer-dashboard.jsp" class="back-button">← Back to Dashboard</a>
            </div>
        </div>
            ...
            <div class="district-percentage-container">
                ...
                <div class="district-cards-wrapper">
                    <% for (Map.Entry<String, Double> entry : districtPercentage.entrySet()) {
                        ...
                        int count = districtCount.get(dName);
                        int districtRankPosition = districtRank.get(dName);
                    ...
                    <div class="district-card <%= performanceClass %>">
                        <div class="district-header">
                            <div class="district-name-large"><%= dName %></div>
    ...
    <script>
        // Division dropdown changed: reload page scoped to the new division
        function onDivisionChange() {
            const div = document.getElementById('divisionFilter').value;
            const base = window.location.pathname;
            if (div && div !== 'ALL') {
                window.location.href = base + '?division=' + encodeURIComponent(div);
            } else {
        ...
        function initializeCharts() {
            ...
            const defaultOptions = {
                ...
                plugins: {
                    legend: {
                        labels: { font: { size: 12 }, padding: 15, color: '#333' }
                    }
                }
            };
                    ...
                    new Chart(classCtx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                ...
                                backgroundColor: [chartColors.primary, chartColors.secondary, chartColors.success, chartColors.danger, chartColors.warning, chartColors.info],
                                borderWidth: 2,
                                borderRadius: 5
                            }]
                        },
            ...
            try {
                ...
                if (schoolChartElement) {
                    ...
                    new Chart(schoolCtx, {
                        type: 'bar',
                        data: {
                            labels: [<% for (Map.Entry<String, Integer> entry : topSchools) { %>'<%= entry.getKey().replace("'", "\\'") %>',<% } %>],
                            datasets: [{
                                label: 'Students',
                        ...
                        options: {
                            ...defaultOptions,
                            indexAxis: 'y',
                            layout: { padding: { left: 150, right: 20, top: 10, bottom: 10 } },
                            plugins: { legend: { display: false } },
                            scales: {
                                x: { beginAtZero: true, ticks: { stepSize: 1, color: '#333' } },
                                y: { ticks: { font: { size: 11 }, padding: 5, color: '#333' }, clip: false }
                            }
                        }
                    });
                }
