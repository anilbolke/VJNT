...
<head>
    ...
    <title>Student Level Distribution - <%= divisionName %></title>
...
<body>
        ...
        <div class="header">
            ...
            <div class="header-content">
                ...
                <div class="header-subtitle">
                    Division: <%= divisionName %> | Detailed Level-wise Breakdown
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function loadData() {
            ...
            let url = '<%= request.getContextPath() %>/division-student-level-distribution?division=' + 
        ...
        function exportToCSV() {
            ...
            if (currentView === 'district') {
                ...
                filename = 'district_level_distribution_' + new Date().toISOString().split('T')[0] + '.csv';
            } else {
        ...
        function searchByUdise() {
            ...
            // Filter current data
            if (currentView === 'district') {
                // If in district view, find which district has this school and switch to school view
                findAndShowSchoolByUdise(udiseInput);
            } else {
                // If already in school view, just filter the schools
        ...
        function findAndShowSchoolByUdise(udiseNumber) {
            ...
            // We need to search across all districts
            // For now, we'll prompt user to select district first or we can make an API call
            // Let's make it search within current district data if available
            ...
            // Search through all districts to find the UDISE
            let foundDistrict = null;
            
            // We need to fetch school data to search - let's do a different approach
            // Just filter current view if we have school data
            if (currentView === 'school') {
                filterSchoolsByUdise(udiseNumber);
            } else {
                // Ask user to first select a district or show message
                alert('Please enter district view first by clicking on a district, then search for UDISE number. Or we can search all schools - fetching data...');
                
                // TODO: Could implement a global search across all districts
                // For now, let's filter district cards if they match
        ...
        function filterSchoolsByUdise(udiseNumber) {
            ...
            // Filter schools that match the UDISE (partial match)
            ...
            // Update breadcrumb to show filter is active
            document.getElementById('breadcrumb').innerHTML = `
                <a href="#" onclick="showDistrictView(); return false;">All Districts</a>
                <span> / </span>
        ...
        function clearUdiseFilter() {
            ...
            if (currentView === 'school' && currentDistrict) {
                // Reload school view without filter
                loadData();
            } else {
