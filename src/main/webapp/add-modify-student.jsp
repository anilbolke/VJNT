...
<%
    ...
    // NOTE: the PEN is deliberately NOT generated here any more.
    // Generating at page render only ever produced a preview: nothing was reserved, so every
    // school with this form open saw the same number, and the one they were shown was not the
    // one they ended up with. It is now allocated at save time and reported back in the
    // success message. See StudentDAO.generateNextPenNumber().
    ...
    if ("save".equals(action)) {
        try {
            ...
            if (isEdit && editStudent != null) {
                ...
                if (submittedPen != null && !submittedPen.equals(editStudent.getStudentPen())) {
                ...
                // Allocate the PEN here, at save time, and ignore whatever the form submitted.
                // The browser's value is only a placeholder now, and trusting it is what let two
                // schools save the same number.
                int maxPenRetries = 5;
                boolean penAssigned = false;
                String finalPen = null;

                for (int i = 0; i < maxPenRetries && !penAssigned; i++) {
                    finalPen = studentDAO.generateNextPenNumber();
                    if (finalPen != null && !studentDAO.isPenNumberExists(finalPen)) {
                        penAssigned = true;
                ...
                if (!penAssigned) {
                    ...
                    if (studentDAO.createStudent(student)) {
                        ...
                        // Reset form for next entry. No PEN is pre-generated for the next student —
                        // it is allocated when that record is saved.
                        editStudent = null;
                    } else {
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="info-box">
                <p><strong>📋 Instructions:</strong> System auto-populates <strong>Division, District, and UDISE Number</strong>. <% if (!isEdit) { %><strong>Student PEN is assigned automatically when you save</strong>, in format TEMPXXXXX (e.g., TEMP12345), and shown in the confirmation message. <% } %>Fill in all other required fields marked with <span style="color: #f44336;">*</span>.</p>
            </div>
            ...
            <form method="POST" action="<%= request.getContextPath() %>/add-modify-student.jsp<% if (isEdit && editStudent != null) { %>?studentId=<%= editStudent.getStudentId() %><% } %>">
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            <label>Student PEN <% if (isEdit) { %><span class="required">*</span><% } %></label>
                            <input type="text" name="studentPen" <%= isEdit ? "required" : "readonly" %>
                                   value="<%= editStudent != null && editStudent.getStudentPen() != null ? editStudent.getStudentPen() : "" %>"
                                   placeholder="<%= isEdit ? "Student PEN" : "Assigned automatically when you save" %>"
                                   style="<%= !isEdit ? "background-color: #f0f2f5; cursor: not-allowed;" : "" %>">
                            <% if (!isEdit) { %>
                            <small style="color: #4CAF50; font-size: 12px; margin-top: 5px; display: block;">
                                ✓ A unique temporary PEN (TEMPxxxxx) is assigned when you save, and shown in the confirmation.
                            </small>
                            <% } %>
                        </div>
                    </div>
