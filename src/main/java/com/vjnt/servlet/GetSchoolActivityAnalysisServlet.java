...
public class GetSchoolActivityAnalysisServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("GetSchoolActivityAnalysisServlet - User: " + user.getUsername());
        System.out.println("GetSchoolActivityAnalysisServlet - District: " + districtName);
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            String schoolSql = "SELECT DISTINCT s.udise_no, sc.school_name " +
                              "FROM students s " +
            ...
            while (rs.next()) {
                ...
                schoolData.addProperty("schoolName", schoolName != null ? schoolName : udiseNo);

                // Get total students in this school
                int totalStudents = getTotalStudents(conn, udiseNo);
                schoolData.addProperty("totalStudents", totalStudents);

                // Get phase-wise analysis
                JsonArray phasesArray = new JsonArray();
                for (int phase = 1; phase <= 4; phase++) {
                    JsonObject phaseData = getPhaseAnalysis(conn, udiseNo, phase);
    ...
    private int getTotalStudents(Connection conn, String udiseNo) throws SQLException {
        String sql = "SELECT COUNT(*) as total FROM students WHERE udise_no = ? AND is_active = 1";
